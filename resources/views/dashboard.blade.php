                    ...
                    <div class="mb-12">
                        <h2 class="text-2xl font-medium text-gray-900 title-font px-4">
                            @if (Auth::user()->is_employer)
                                All active jobs Created
                            @else
                                All active jobs applied to
                            @endif
                            ({{ $listings->where('is_active', true)->count() }})
                        </h2>
                    </div>
                    <div class="-my-6">
                        @foreach($listings->where('is_active', true)  as $listing)
                            <a
...
<div class="mb-12">
    <h2 class="text-2xl font-medium text-gray-900 title-font px-4">
        @if (Auth::user()->is_employer)
            All inactive jobs Created
        @else
            All inactive jobs applied to
        @endif
        ({{ $listings->where('is_active', false)->count() }})
    </h2>
</div>
<div class="-my-6">
    @foreach($listings->where('is_active', false)  as $listing)
        <a
            href="{{ route('applications.show', $listing->slug) }}"
            class="py-6 px-4 flex flex-wrap md:flex-nowrap border-b border-gray-100
            {{ $listing->is_highlighted ? 'bg-yellow-100 hover:bg-yellow-200' : 'bg-white hover:bg-gray-100' }}"
